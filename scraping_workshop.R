...
read_html(url) %>% 
  html_node(".metascore_w.user.large.game.positive") %>% 
  #html_node(".score_summary div div:nth_child(2)") %>% 
  html_text


scrape_fun <- function(page){
  
  url <- paste0("https://www.metacritic.com/game/playstation-5/elden-ring/user-reviews&page=", page)
  
  
  reviews <- read_html(url) %>% 
    html_nodes(".inline_expand_collapse.inline_collapsed span") %>% 
    html_text(trim=T) %>% 
    str_squish
  
  dates <- read_html(url) %>% 
    html_nodes(".review_critic .date") %>% 
    html_text(trim=T) %>% 
    str_squish
  
  df <- data.frame(reviews, dates)
...
reviews_df <- map_df(seq(1, 33), scrape_fun)
...
read_html("https://openstates.org/ut/") %>% 
  html_nodes(".state-overview-chambers div:nth-child(2)") %>% 
  html_text(trim = T) %>% 
  str_squish
...
download_pdf <- function(page){
  ...
  bill_versions <-
    mutate(bill_versions,
           file_name = paste0(bill_num, "-", 
                              `Bill Text Versions`, ".pdf"))
